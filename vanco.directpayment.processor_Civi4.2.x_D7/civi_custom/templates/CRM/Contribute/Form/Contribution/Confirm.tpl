        ...
        <div class="display-block">
            ...
            {if $is_recur}
                ...
                {else}
                    ...
                    {/if}
                    <p>{ts}Your initial contribution will be processed once you complete the confirmation step.{/ts}</p>
                {/if}
            {/if}
            {if $is_pledge }
...
{* Modified for Vanco *}
    ...
    {if $contributeMode eq 'direct' and ! $is_pay_later and $is_monetary and ( $amount GT 0 OR $minimum_fee GT 0 )
				and ($credit_card_number or $bank_account_number or $account_number )}
