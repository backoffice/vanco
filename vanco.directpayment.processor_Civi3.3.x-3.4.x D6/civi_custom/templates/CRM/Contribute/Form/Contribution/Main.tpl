...
cj(document).ready(function(){
	cj('#installments').blur(function(){
		calculateAmount();
	});
	cj('#amount_other').blur(function(){
		calculateAmount();
	});
	cj("input:radio[name=amount]").change(function(){
		calculateAmount();
	});
	...
	function calculateAmount( ) {		
		var amountValue = cj("input:radio:checked[name=amount]").val( );
		var installment = cj('#installments').val();
		if ( amountValue != 'amount_other_radio' && !cj('#amount_other').val( ) ) {
		    var amount = new Array();
		    {/literal}
		    {foreach from=$amountArray item=item key=id}
		        {literal}amount['{/literal}{$id}{literal}'] = {/literal}'{$item}';
		    {/foreach}
		    {literal}
		    var amt = amount[amountValue].replace(",","");
		    var total = (amt * installment).toFixed(2);
		    } else {	
		    var value = cj('#amount_other').val();
		    if( !value ) {
		    	value = 0;
		    }
		    var val = value.replace(",","");
		    var total = (installment * val).toFixed(2);
    		}
		if ( installment ) {
		    cj('.totalamount').html('Calculated Amount is $ '+total);
		}
	}
});
